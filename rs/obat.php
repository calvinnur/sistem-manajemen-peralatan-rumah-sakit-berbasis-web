...
<div class="content-table">
    <img src="img/logo2.png">
    <p style="position:relative; left:10px; font-size:20px;">List Obat</p>
    ...
    <?php 
    $pagination = $call->pagination("select * from db_barang where kategori = 'obat'",$page,10);
    $total = $pagination['total_page'];
    $limit = $pagination['limit_data'];
    $offset = $pagination['offset'];

    $query = $call->querys("select * from db_barang where tanggal like ('%$waktu%') and kategori = 'obat' order by tanggal desc limit $offset,$limit");
    ...
    if($row == 0){
        ...
        <tr>
            ...
            <td style="text-align: center;"><img src="img/edit.png" onclick="ubah_data(<?php echo $show['id']?>)" style="width: 25px; height:25px; position:relative; left:25px; top:-5px; cursor:pointer;">
            <a onclick="javascript: return confirm('Apakah anda yakin untuk menghapus data ini?')"  href="delete_obat.php?id=<?php echo $show['id']?>"><img  style="width: 25px; height:25px; position:relative; left:20px; top:-5px;" src="img/trash.png"></a>
        </tr>
        <?php        
        }
    }
    ...
    if($row !== 0){
        ...
        <div class="module-button" style="margin-left: 20px;">
            <a href="obat.php?page=<?php echo $a ?>">First</a>
        </div>
            <?php }?>
    <div class="module-button" <?php if ($a == $page) echo 'style="background:#629e21; "'; ?>>
        <a <?php if ($a == $page) echo 'style="color:white;"'; ?> style="left: 20px;" href="obat.php?page=<?php echo $a?>"><?php echo $a?></a>
     </div>
     <?php if ($a == $total) { ?>
        <div class="module-button">
            <a href="obat.php?page=<?php echo $a?>">Last</a>
        </div>
    <?php } ?>
...
<form method="POST" action="update_obat.php">
    <div class="modal " id="obat">
      <input type="hidden" id="id" name="id">
        <h2>Ubah data</h2><i onclick="ubah_data()" class="fas fa-times" style="float: right; margin-right:30px; margin-top:-50px; cursor:pointer;"></i>
        <div class="garis" style="margin-left:0px; margin-top: 0px; width:100%;"></div><br>
        <label>Tanggal</label><br>
        ...
        <button type="submit" class="submit">Simpan</button><button type="button"  onclick="ubah_data()" class="cancel">Tutup</button>
    </div>
</form>
